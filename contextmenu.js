...
import { getGoalElements, getGoalParentElement } from "./util";
...
function openContextMenu(sendResponse) {
  sendResponse({
    ...
    yesterhide: LocalStorage.loadYesterHide(activeGoal.dataset),
    hideOn: LocalStorage.loadHideOn(activeGoal.dataset),
    hideAfter: LocalStorage.loadHideAfter(activeGoal.dataset),
  });
}
...
function saveGoalInfo(msg) {
  ...
  LocalStorage.storeHideAfter(msg);
  LocalStorage.storeYesterHide(msg);
}
